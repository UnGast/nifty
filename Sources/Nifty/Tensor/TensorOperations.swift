...
extension Tensor where Element: BinaryFloatingPoint {
  /// - Returns: dot product
  public func dot(_ other: Tensor<Element>) -> Tensor<Element> {
    precondition(
      dim == 1 && other.dim == 1,
      "dot(): both tensors must have only one dimension, are: \(dim), \(other.dim)")
    precondition(
      numel == other.numel,
      "dot(): the tensors must have the same number of elements, are: \(numel), \(other.numel)")

    return Tensor([1], [(0..<numel).reduce(into: Element(0)) {
      $0 += data[$1] * other.data[$1]
    }])
  }
  ...
  public func matmul(_ other: Tensor<Element>) -> Tensor<Element> {
    guard dim == 2 else {
      fatalError("matmul: both tensors must have 2 dimensions, first one has: \(dim)")
    }
    guard other.dim == 2 else {
      fatalError("matmul: both tensors must have 2 dimensions, second one has: \(other.dim)")
    }
